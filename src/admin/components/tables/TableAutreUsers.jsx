...
function TableAutreUsers({titre,bouttonName,route,role}) {
    ...
    const RemoveData = async ()=>{
      ...
      try {
        const reponse = await fetch(
          `http://localhost:3000/api/users/${currentObjet._id}`,
          {
            method: "DELETE",
          }
        );
        ...
        if (reponse.ok) {
           closeModal()
           Notification.reussite(`${role} supprimer avec succés.`)
  ...
  if(erreur){
    ...
    return (
        ...
        {isLoading ? (
                    ...
                    <td>
                      <div className="action">
                        ...
                        >
                          ...
                          <Link to = {`/admin/utilisateurs/receptionniste/profile/${element._id}`}>
                          <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width={20}
                          height={20}
          ...
          >
            <p> Etes vous sur des vouloir supprimer le {role} {`${currentObjet.nom} ${currentObjet.prenom}`} ? </p>
          ...
          >
            <div className="modal_profil" >
              ...
              <hr />
              <picture>
              <img src={currentObjet.photo? currentObjet.photo : profil } alt="profilImage"/>
              </picture>
              
               
              <div className="modal_profil_info">
                <div className="ligne"><p>Nom : </p> <span>{currentObjet && currentObjet.nom}</span></div>
                 <div className="ligne"><p>Prénom : </p> <span>{currentObjet && currentObjet.prenom}</span></div>
                 <div className="ligne"> <p>Sexe :</p><span>{currentObjet && currentObjet.sexe}</span>  </div>
                 <div className="ligne"> <p>Date de Naissance :</p> <span>{currentObjet && new Date( currentObjet.dateNaissance).toLocaleDateString() }</span> </div>
                 
                 <div className="ligne"> <p>Adresse : </p> <span>{currentObjet && currentObjet.adresse}</span></div>
                 <div className="ligne"> <p>Numéro De Téléphone :</p> <span>{currentObjet && currentObjet.numeroTel}</span></div>
                 <div className="ligne"> <p>Numéro de Sécurité Sociale :</p> <span>{currentObjet && currentObjet.numeroSecurite}</span></div>
...
export default TableAutreUsers;