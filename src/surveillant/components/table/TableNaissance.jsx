    ...
    const fetchRdv = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:3000/api/bebe/surveillance/naissance/tout`
        );

        const surveillances = await response.json();
        //rendeVous.sort((a, b) => new Date(a.date) - new Date(b.date));
        const resultRecherche = surveillances.filter((element) => {
          const bebe = `${element.prenomEnfant}`.toLowerCase();
          const dame = `${element.dame}`.toLowerCase();

          const prefix = searchTerm.toLowerCase();

          return bebe.startsWith(prefix) || dame.startsWith(prefix);
        });
        setTotalPages(
          !recherche
            ? Math.ceil(surveillances.length / nombreElementPage)
            : Math.ceil(resultRecherche.length / nombreElementPage)
        );
        setData(!recherche ? surveillances : resultRecherche);
      } catch (e) {
  ...
  return (
    <div className="consultation_table">
      ...
      <div
        ...
        className="doc_print"
      >
        {currentDoc && <Naissance data={currentDoc} reference={componentRef} />}
      </div>
      <div className="consultation_table_btn">
        <Link
          to="/surveillant/naissance/ajouter"
          className="btn_ajout"
          style={{
            textDecoration: "none",
            color: "#fff",
            ...
            <table className="table">
              ...
              <tbody>
                {currentData.map((element) => (
                  <tr key={element._id}>
                    <td>{`${element.dame}`}</td>
                    <td>{element.prenomEnfant} </td>
                    <td>{element.sexeEnfant}</td>
                    ...
                    <td>
                      <div className="action">
                        ...
                        <Link
                          to={`/surveillant/naissance/modifier/${element._id}`}
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            width={20}
