   ...
   const indiceDepart = (currentPage - 1) * nombreElementPage;
    ...
    const RemoveData = async ()=>{
     ...
     const passeRendeVous = async(id) =>{
      closeModal1()
      try {
        const response = await fetch(`http://localhost:3000/api/rendez-vous/confirmer/${id}`, {
    ...
    return (
        ...
        <select name="motif" id="motif"   onChange={(e)=>setMotif(e.target.value)}>
         ...
         <Link to="/medecin/rendez-vous/archives" style={{textDecoration:'none'}} className='btn_archive'> 
      ...
      ( 
        <>
       <h3 style={{textAlign:"center",color:"darkgreen"}}> {motif !== "" ?`Rendez-vous pour ${motif}` :"rendez-vous d'aujourd'huit"}</h3>
     ...
     <div className="consultation_table__content">
        ...
        <tbody>
             ...
             {motif !=="" ?
              currentData.filter(element => element.motif === motif && !element.status ).map((element)=>(
                ...
                <td> {new Date(element.date).toLocaleDateString()=== new Date().toLocaleDateString()? "Aujourd'huit" : new Date(element.date).toLocaleDateString()}  </td>
                
                
                <td style={{color:`${element.status? "darkgreen":"darkred"}`}} >{element.status? "Passé" : "En attente"}</td>
                <td>
               ...
               currentData.filter(element =>new Date(element.date).toLocaleDateString() === new Date().toLocaleDateString() && !element.status ).map((element)=>(
          ...
          >
            ...
            <div className="modal_btn">
              <button onClick={closeModal1}>Fermer</button>
               ...
               :
                <button onClick={(e)=>passeRendeVous(currentObjet._id)}>Confirmé</button>
