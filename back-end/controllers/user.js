...
exports.modifierUser = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Profil utilisateur modifié avec succès." });
  } catch (error) {
    res
      .status(500)
      .json({
...
exports.signUp = async (req, res, next) => {
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(409)
        .json({ error: "Un utilisateur avec le même prénom existe déjà." });
    }
    ...
    if (req.body.role !== "Médecin") {
      ...
      try {
        ...
        return res
          .status(201)
          .json({
            message: "Utilisateur enregistré avec succès en tant que médecin.",
            user,
          });
      } catch (error) {
        ...
        return res
          ...
          .json({
            error:
              "Une erreur est survenue lors de l'enregistrement du médecin.",
          });
      }
    }
  } catch (error) {
